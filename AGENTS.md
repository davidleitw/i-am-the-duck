...
`skills/duck/SKILL.md` is read by the agent at every session start, and its words end up in the agent's replies. So: no jokes, no vocabulary invented for this project, no names invented for this plugin. Ordinary technical words the reader already uses are fine. The duck and the story live in `README.md`. Keep the file under 50 lines; an example is one line.
