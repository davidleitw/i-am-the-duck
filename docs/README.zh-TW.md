...
**三題下來，Opus 5（high）配 Duck：回覆字元與輸出 token 少了三成到五成，耗時與費用少了兩成到四成，盲讀者判定答案的正確程度跟 baseline 相當。**

![依題目比較，有無 Duck 的回覆字元與輸出 token，Opus 5 high。](../evals/assets/initial-results.svg)

同樣的發現，少很多閱讀量。Duck 的回覆先講結論，只在分岔會影響結果的地方畫流程圖，照提問者的問題順序回答，並且分開說哪些是看過程式碼確認的、哪些只是讀文件讀來的。

最新一題來自一次真實的對話：agent 用審查報告的編號和規格章節代號回報設計現況，問的人只好要求講白話。每一份回覆都在揭曉分組之前先交給盲讀者評分，而唯一被每位盲讀者都判為「講得白」的，是 Duck 的回覆。[數據與盲讀](../evals/results/README.md)。
